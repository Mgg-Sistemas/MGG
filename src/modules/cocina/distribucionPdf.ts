...
export async function verDistribucionPdf(items: ResumenDistribucion[], cab: CabeceraDistribucion): Promise<void> {
  ...
  doc.text('CONTROL DE DISTRIBUCIÓN · MODELO EOQ', W / 2, y + 18, { align: 'center' });
  doc.setTextColor(40, 40, 40); doc.setFontSize(11);
  doc.text(`Comedor industrial · Cocina: ${cab.cocina}`, W / 2, y + 34, { align: 'center' });
  doc.setTextColor(90, 90, 90); doc.setFont('helvetica', 'normal'); doc.setFontSize(9);
  doc.text(`Mercado #${cab.mercado} · ${fmtDia(cab.desde)} → ${fmtDia(cab.hasta)}`, W / 2, y + 48, { align: 'center' });
  ...
  doc.text(
    `Víveres: ${t.viveres}   ·   Por reponer: ${t.reordenar}   ·   En alerta: ${t.alerta}   ·   Consumido: ${num(t.consumoTotal)}   ·   Mermas: ${num(t.mermas)}`,
    MARGIN, y,
  );
  doc.setFont('helvetica', 'normal'); doc.setTextColor(90, 90, 90); doc.setFontSize(8.5);
  doc.text(
    `Parámetros de compra: ordenar $ ${num(cab.params.costoOrden)} · almacenar $ ${num(cab.params.costoAlmacenar)} por unidad/año · entrega ${num(cab.params.leadTimeDias)} días`,
    MARGIN, y + 12,
  );
  doc.setTextColor(0, 0, 0);
  y += 22;
  ...
  autoTable(doc, {
    startY: y + 6,
    head: [['VÍVER', 'UNIDAD', 'STOCK', 'CONSUMO', 'PROM./DÍA', 'RATIO x COM.', 'D ANUAL', 'LOTE EOQ', 'REORDEN', 'CICLO', 'ESTADO']],
    body: orden.length
      ? orden.map((i) => [
  ...
  for (const i of orden.filter((x) => x.filas.length)) {
    ...
    doc.text(`Registro diario · ${i.nombre} (${i.unidad})`, MARGIN, y);
    doc.setFont('helvetica', 'normal'); doc.setFontSize(8);
    doc.setTextColor(90, 90, 90);
    doc.text(
      `Stock ${num(i.stock)} · consumo ${num(i.consumoTotal)} (${num(i.promedioDia)}/día) · comensales ${num(i.comensales)} · ${estadoTexto(i.estado)}`,
      MARGIN, y + 11,
    );
    doc.setTextColor(0, 0, 0);
    autoTable(doc, {
      startY: y + 18,
      head: [['FECHA', 'TURNO', 'INV. INICIAL', 'ENTRADAS', 'SALIDAS', 'TEÓRICO', 'MERMA', 'FÍSICO', 'COMENSALES', 'RATIO', 'ESTADO']],
      body: i.filas.map((f) => [
  ...
  doc.text(
    'Lote EOQ = raíz de (2 × demanda anual × costo de ordenar ÷ costo de almacenar). Reorden = consumo de los días de entrega.',
