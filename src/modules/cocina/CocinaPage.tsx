  ...
  return (
    <Modal title="Añadir movimiento · Cocina" size="lg" onClose={() => !saving && onClose()} footer={
      ...
      <form id="cocina-add" onSubmit={submit}>
        ...
        <div className="form-row">
          <label>Víveres consumidos <span className="muted" style={{ fontWeight: 400 }}>(todo el inventario · categorías Víveres y Proteína · {num(viveres.length)} productos{nSel > 0 ? ` · ${num(nSel)} elegido(s)` : ''})</span></label>
          ...
          <div style={{ display: 'grid', gap: '.4rem', maxHeight: 340, overflowY: 'auto', paddingRight: '.15rem' }}>
            {!filtrados.length && <div className="muted" style={{ padding: '1rem', textAlign: 'center' }}>{viveres.length ? 'Ningún víver coincide con la búsqueda.' : 'No hay productos de categorías Víveres o Proteína en el inventario.'}</div>}
            {filtrados.map((v) => {
