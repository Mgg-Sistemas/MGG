...
export async function descargarReportePesoCentrosPdf(data: ReportePesoData): Promise<void> {
  ...
  for (const centro of ordenCentros) {
    ...
    autoTable(doc, {
      ...
      // "TOTAL" va en la columna ITEM sin partirse (overflow visible) y el valor
      // queda alineado bajo la columna PESO, como los pesos de arriba.
      foot: [['TOTAL', n2(totalCentro), '', '']],
      theme: 'grid',
      headStyles: { fillColor: BRAND, textColor: 255, fontSize: 8.5, fontStyle: 'bold' },
      footStyles: { fillColor: GRIS, textColor: 20, fontStyle: 'bold', fontSize: 9, overflow: 'visible' },
  ...
  autoTable(doc, {
    ...
    foot: [[n2(granTotal), 'TOTAL DE KG DE CASITERITA A RECEPCIONAR']],
    theme: 'grid',
