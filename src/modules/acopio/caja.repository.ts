...
export interface CajaMovimientoInput {
  ...
  /** Kg de material comprado: suman al Saldo en Kg acumulado. */
  compra_material_kg?: number;
  /** Tasa $/Kg de esa compra (informativa). */
  compra_material_tasa?: number;
  kg_recibidos?: number;
...
export async function listCajaMovimientos(cajaId?: string, centroNombre: string | null = CENTRO_ACOPIO_DEFECTO): Promise<CajaMovimiento[]> {
  ...
  return (data ?? []).map((row) => {
    ...
    // Los Kg comprados (compra de material) SUMAN al saldo, igual que los Kg cerrados.
    saldoKg += num(m.kg_cerrados) + num(m.compra_material_kg) - num(m.kg_recibidos);
...
export function resumirCaja(movs: CajaMovimiento[]): CajaResumen {
  const r = movs.reduce(
    (a, m) => ({
      ...
      compraMaterialKg: a.compraMaterialKg + num(m.compra_material_kg),
      kgRecibidos: a.kgRecibidos + num(m.kg_recibidos),
    }),
    { usdEntregado: 0, kgCerrados: 0, facturados: 0, gastos: 0, nominas: 0, traslado: 0, compraMaterial: 0, compraMaterialKg: 0, kgRecibidos: 0 },
  );
  const saldoUsd = r.usdEntregado - r.facturados - r.gastos - r.nominas - r.traslado - r.compraMaterial;
  // Los Kg comprados (compra de material) SUMAN al Saldo en Kg, igual que los Kg cerrados.
  const saldoKg = r.kgCerrados + r.compraMaterialKg - r.kgRecibidos;
  // F3 = (G3 + H3 + I3) / E3
...
export async function crearMovimientoCaja(input: CajaMovimientoInput, actor: string, actorName?: string | null): Promise<CajaMovimiento> {
  ...
  const payload = {
    ...
    compra_material_kg: num(input.compra_material_kg),
    compra_material_tasa: num(input.compra_material_tasa),
...
export async function cerrarYAbrirCaja(input: {
  ...
  // 1) SALDO EN KG del CENTRO PRINCIPAL → RECEPCIONES (paso intermedio). Para el
  //    CENTRO, lo que entra a la recepción es el SALDO EN KG de casiterita de la caja
  //    que se cierra (kg_cerrados − kg_recibidos), NO los recibidos. Los ALIADOS, en
  //    cambio, entran por sus Kg RECIBIDOS por MGG (ver cerrarYAbrirCajaAliado). El
  //    ingreso al inventario se hará luego desde Recepciones. Solo si el saldo es > 0.
  ...
  if (saldoKg > 0) {
    ...
    kgAInventario = true; // (flag para el resumen del cierre: hubo Kg que pasaron a Recepciones)
  }
