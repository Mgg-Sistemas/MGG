...
const HEAD = [
  ...
  'Gastos', 'Traspaso de Caja', 'Saldo $ Usd', 'Kg Recib. MGG', 'Saldo Kg casiterita',
];
/** Gastos unificados (gastos + nómina) de una fila. */
const gastosUnif = (r: MovAcopioRow) =>
  (r.gastosGt == null && r.nominasGt == null) ? null : (Number(r.gastosGt) || 0) + (Number(r.nominasGt) || 0);
...
async function construirDoc(rows: MovAcopioRow[], meta: MovAcopioMeta = {}) {
  ...
  const body = rows.map((r) => [
    ...
    fmtUsd(gastosUnif(r)), fmtUsd(r.trasladoCaja), fmtUsd(r.saldoUsd), fmtNum(r.kgRecibidosMgg), fmtNum(r.saldoKgCasiterita),
  ]);
...
export async function descargarMovAcopioPdf(rows: MovAcopioRow[], meta: MovAcopioMeta = {}): Promise<void> {
  previewPdfDoc(await construirDoc(rows, meta), `${NOMBRE}.pdf`);
}
...
export async function descargarMovAcopioExcel(rows: MovAcopioRow[]): Promise<void> {
  ...
  const filas = rows.map((r) => [
    ...
    gastosUnif(r) ?? '', r.trasladoCaja ?? '', r.saldoUsd, r.kgRecibidosMgg ?? '', r.saldoKgCasiterita,
  ]);
  ...
  (ws as Record<string, unknown>)['!cols'] = [
    ...
    { wch: 12 }, { wch: 15 }, { wch: 13 }, { wch: 13 }, { wch: 16 },
  ];
